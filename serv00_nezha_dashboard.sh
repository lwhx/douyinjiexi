...
build() {
  ...
  yellow "克隆项目成功"
  wget https://github.com/nezhahq/admin-frontend/releases/download/v1.8.0/dist.zip >/dev/null 2>&1
  unzip dist.zip >/dev/null 2>&1
  rm -rf admin-dist
  mv dist admin-dist
  rm -rf dist.zip

  wget https://github.com/hamster1963/nezha-dash-v1/releases/download/v1.23.0/dist.zip >/dev/null 2>&1
  unzip dist.zip >/dev/null 2>&1
  rm -rf user-dist
  mv dist user-dist
  rm -rf dist.zip
